...
# Standalone: mock sglang envs — disable JIT radix-select by default.
class _MockEnvs:
    def __init__(self):
        self._val = False
    ...
    def override(self, v):
        self._val = v
        return self  # allow `with envs.XXX.override(v):` context
    def __enter__(self): return self
    def __exit__(self, *a): pass

envs = type("_envs", (), {
    "SGLANG_OPT_USE_MINIMAX_DECODE_TOPK_RADIX": _MockEnvs(),
})()
...
def flash_decode_with_topk_idx(
    ...
    if use_dense_main_attn:
        try:
            ...
            raise ImportError(
                "use_dense_main_attn requires sglang.jit_kernel.minimax_decode_topk. "
                "Install sglang or set use_dense_main_attn=False for standalone mode."
            )
    ...
    if _skip_block_topk:
        ...
        try:
            ...
            raise ImportError(
                "JIT topk requires sglang.jit_kernel.minimax_decode_topk. "
                "Install sglang or disable envs.SGLANG_OPT_USE_MINIMAX_DECODE_TOPK_RADIX."
            )
