...
``index_topk``). Adapted to a paged KV cache: the KV page size equals the sparse
block size (128), so one selected block maps to exactly one page.

Main K/V cache layout: ``(num_blocks, 2, 128, num_kv_heads, head_dim)``  K=[:,0] V=[:,1]
...
selected blocks with a separate merge step.
"""
...
from kernels.common import SPARSE_BLOCK_SIZE, _FP8_DTYPES, is_pdl_supported
...
def minimax_m3_sparse_attn_decode(
    ...
    # split-K over the selected blocks; chunk count is shape-constant.
    TARGET_GRID = 256
